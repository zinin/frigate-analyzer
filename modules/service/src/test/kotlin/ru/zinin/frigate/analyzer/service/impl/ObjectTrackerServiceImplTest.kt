...
class ObjectTrackerServiceImplTest {
    ...
    private val transactionalOperator = mockk<TransactionalOperator> {
        every { execute(any<TransactionCallback<Any>>()) } answers {
            ...
            // executeAndAwait wraps the suspend lambda inside a Mono;
            // the TransactionalOperatorImpl would subscribe to it — we must too.
            val value = if (rawResult is Mono<*>) {
                rawResult.blockOptional().orElse(null)
            } else {
                rawResult
            }
            if (value != null) {
    ...
    fun `out-of-order older recording matches existing track via updateOnMatch with original lastSeenAt`() = runTest {
        ...
        coVerify(exactly = 1) {
            repo.updateOnMatch(
                existing.id!!, any(), any(), any(), any(),
                Instant.parse("2026-04-27T12:00:00Z"), recId,
            )
        }
    }

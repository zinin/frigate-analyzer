...
class WatchRecordsTaskTest {
    ...
    fun `backoff resets after 5 consecutive successes after failures`() =
        runTest {
            ...
            // (see onPollCompleted in WatchRecordsTask — maybeResetBackoff is called only on events > 0).
    ...
    // The 10-field state is set via internal vars — works because this test file is in the same Kotlin module.
    ...
    fun `registerAllDirs failure leaves watchService null and bookkeeps registration failure`() =
        runTest {
            ...
            assertEquals(1, task.consecutiveRegistrationFailures)
            assertEquals(2, task.consecutiveFailures)
            assertTrue(task.lastFailure is java.io.IOException)
        }
    ...
    fun `5 consecutive registerAllDirs failures push health to DOWN via branch 2`() =
        runTest {
            val task = newTask(dispatcher = StandardTestDispatcher(testScheduler))
            task.startupAt = Instant.parse("2026-05-23T12:00:00Z")
            ...
            assertTrue(
                task.consecutiveRegistrationFailures >= 5,
                "Got ${task.consecutiveRegistrationFailures}",
            )

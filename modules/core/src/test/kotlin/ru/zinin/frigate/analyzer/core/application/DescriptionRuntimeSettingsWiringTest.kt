...
/**
 * Проводка SPI рантайм-настроек. Автоконфигурация `ai-description` регистрирует in-memory-дефолт под
 * `@ConditionalOnMissingBean`, поэтому ошибка в условиях бина из `core` ничего не ломает: приложение
 * стартует, `/ai` работает, а выбор владельца молча пропадает на каждом рестарте. Ассерт на тип бина —
 * единственное, что такую подмену удержит.
 *
 * `ApplicationContextRunner`, а не `@SpringBootTest`: нужна ровно автоконфигурация фичи и её условия,
 * без R2DBC и testcontainers — та же причина, что у `SignalLossConfigConflictIntegrationTest`.
 */
class DescriptionRuntimeSettingsWiringTest {
    @TempDir
    ...
    private val runner =
        ApplicationContextRunner()
            ...
            .withUserConfiguration(SpiStubConfig::class.java, AppSettingsDescriptionRuntimeSettings::class.java)
    ...
    fun `the app_settings implementation wins over the in-memory default`() {
        runner
            ...
            .run { context ->
                ...
                assertThat(context).doesNotHaveBean(InMemoryDescriptionRuntimeSettings::class.java)
                val bean = context.getBean(DescriptionRuntimeSettings::class.java)
                assertIs<AppSettingsDescriptionRuntimeSettings>(bean)
    ...
    fun `the implementation stays inside the component-scanned tree`() {
        val type = AppSettingsDescriptionRuntimeSettings::class.java

 ...
 *
 * Clock-skew handling: if `maxRecordTs > now`, the gap is clamped to zero (and a single
 * warn is logged per call). The camera is then treated as healthy.
 */
...
fun decide(
    ...
    return when {
        ...
        prev == null && overThreshold && cfg.inGrace -> {
            Decision(
        ...
        prev is CameraSignalState.Healthy && overThreshold && cfg.inGrace -> {
            Decision(
                newState = CameraSignalState.SignalLost(obs.maxRecordTs, notificationSent = false),
                event = null,
            )
        ...
        prev is CameraSignalState.SignalLost && overThreshold && !prev.notificationSent && !cfg.inGrace -> {
            Decision(
        ...
        prev is CameraSignalState.SignalLost && overThreshold && prev.notificationSent -> {
            Decision(
            ...
            error(
                "Unreachable: prev=$prev, overThreshold=$overThreshold, inGrace=${cfg.inGrace}",
            )
        }
    }
}

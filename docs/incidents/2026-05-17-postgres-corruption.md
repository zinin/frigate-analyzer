...
- **Устарело с 2026-08-03 (ветка `perf/watch-records-registration`):** автоматического наверстывания после рестарта больше нет — `FirstTimeScanTask` стал opt-in (`DISABLE_FIRST_SCAN=true` по умолчанию) и ограничен окном `FIRST_SCAN_PERIOD`. Пропуск за время простоя теперь восстанавливается вручную; процедура — в `docker/deploy/.env.example`, секция `# --- Records watcher ---`

...
> **STATUS: RESOLVED** (2026-05-26) — closed by PR for #29. Internal Jackson usage migrated to `tools.jackson` (Jackson 3). `JacksonConfiguration` now explicitly governs WebFlux REST wire-format via `WebFluxJacksonCodecConfigurer`. Jackson 2 remains as transitive dependency of springdoc-openapi and other transitive consumers (YAML loaders, etc.) only — documented in `JacksonConfiguration.kt` KDoc. Spring Boot 4 does NOT ship a `spring-boot-jackson2` compat starter.

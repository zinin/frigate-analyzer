...
| Макс. кадров в запросе | `max-frames: 10` (кратно MAX_MEDIA_GROUP_SIZE) | Контроль latency и стоимости; берём первые `N` по `frameIndex`. |
| Два timeout-а | `queue-timeout` (ожидание слота семафора) + `timeout` (собственно describe + retry) | Разделяем диагностику: очередь vs работа. |
...
**Создание бина с условиями (`@Component` подход, без отдельного `@Configuration`):**

Модуль регистрируется через `AiDescriptionAutoConfiguration` (см. §3.1) — там только `@EnableConfigurationProperties` и `@ComponentScan`. Сами бины:

```kotlin
@Component
@ConditionalOnProperty("application.ai.description.enabled", havingValue = "true")
class ClaudePromptBuilder
// то же для ClaudeResponseParser, ClaudeImageStager, ClaudeAsyncClientFactory, ClaudeExceptionMapper

@Component
@ConditionalOnProperty("application.ai.description.enabled", havingValue = "true")
class DefaultClaudeInvoker(...)
// также с @ConditionalOnProperty("application.ai.description.provider", havingValue = "claude")

@Component
@ConditionalOnProperty("application.ai.description.enabled", havingValue = "true")
class ClaudeDescriptionAgent(...) : DescriptionAgent
// также с @ConditionalOnProperty("application.ai.description.provider", havingValue = "claude")
```

Хелперы условны только по `enabled=true` (не по provider), т.к. могут переиспользоваться будущими провайдерами.
Agent/Invoker условны дополнительно по `provider=claude`.

При `enabled=false` — бинов нет. `ObjectProvider<DescriptionAgent>.getIfAvailable()` вернёт `null`, код facade работает без ветвлений в hot-path.

### 3.1 Auto-configuration модуля

Поскольку Spring Boot сканирует `ru.zinin.frigate.analyzer.core`, бины модуля `ai-description` в пакете `ru.zinin.frigate.analyzer.ai.description` **не будут** автоматически подхвачены.

Решение — auto-configuration по паттерну `telegram` модуля:

```kotlin
// modules/ai-description/src/main/kotlin/.../ai/description/config/AiDescriptionAutoConfiguration.kt
@AutoConfiguration
  ...
  ai:
    description:
      ...
      common:
        ...
        max-frames: ${APP_AI_DESCRIPTION_MAX_FRAMES:10}      # top-N по frameIndex
        queue-timeout: ${APP_AI_DESCRIPTION_QUEUE_TIMEOUT:30s}  # ожидание слота семафора
        timeout: ${APP_AI_DESCRIPTION_TIMEOUT:60s}           # describe + retry
        max-concurrent: ${APP_AI_DESCRIPTION_MAX_CONCURRENT:2}

      claude:
        oauth-token: ${CLAUDE_CODE_OAUTH_TOKEN:}
        model: ${CLAUDE_MODEL:opus}
        cli-path: ${CLAUDE_CLI_PATH:}                        # пусто = SDK ищет в PATH через which
        working-directory: ${CLAUDE_WORKING_DIR:${application.temp-folder}}  # обязательный для SDK
        proxy:
...
- `startup-timeout` удалён — в SDK 1.0.0 такого поля нет (проверено по sources jar `org.springaicommunity:claude-code-sdk:1.0.0`).
- `working-directory` добавлен — `ClaudeClient.AsyncSpec.workingDirectory(Path)` **обязателен** (SDK бросает `IllegalArgumentException("workingDirectory is required")`). По умолчанию берём `application.temp-folder` — там уже есть права у appuser и удаляемые временные файлы.
- `cli-path` теперь передаётся в `ClaudeClient.AsyncSpec.claudePath(String)` (а не в `CLIOptions`). Пусто — `Query.isCliInstalled()` + `which claude`.
- `queue-timeout` добавлен — отдельный бюджет на acquire семафора (см. §5).
- `max-frames` добавлен — ограничение "до 10 кадров" из §2 становится конфигом.
...
data class DescriptionProperties(
    val enabled: Boolean,
    val provider: String,   // без @NotBlank — при disabled может быть пусто
...
data class ClaudeProperties(
    ...
    val model: String,
    val cliPath: String,         // пусто = полагаемся на PATH
    val workingDirectory: String,  // обязателен для SDK
...
`ClaudeProperties` активен только вместе с autoconfig'ом, который условен по `enabled=true` — иначе properties не создаются, и невалидные claude.* поля при выключенной фиче не блокируют старт.

### Стартовая валидация в `ClaudeDescriptionAgent.init` блоке
 ...
 * Живёт в api/ пакете (а не claude/), т.к. это модульный контракт, не
 * claude-специфика: будущие провайдеры (OpenAI/Gemini) будут использовать
 * ту же абстракцию.
 */
...
class ClaudeDescriptionAgent(
    ...
    override suspend fun describe(request: DescriptionRequest): DescriptionResult {
        val acquired = try {
            withTimeout(commonProperties.queueTimeout.toMillis()) { semaphore.acquire() }
        } catch (e: TimeoutCancellationException) {
            throw DescriptionException.Timeout(cause = e) // очередь переполнена
        }
        try {
            return try {
...
Два timeout-а: `queueTimeout` на acquire, `timeout` на саму работу (stage + retry loop). Оба `TimeoutCancellationException` маппятся в `DescriptionException.Timeout` на границе `describe()`, чтобы Telegram-слой получал уже нормализованный тип из sealed-иерархии.
...
- **`ClaudePromptBuilder`** — system-инструкция на `language`, требование
  ...
  **Сортировка:** `request.frames.sortedBy { it.frameIndex }` перед `zip(framePaths)` —
  stager возвращает пути уже отсортированными, но frames на входе могут быть
  неупорядочены (`ConcurrentHashMap.values()`). Для unknown language кодов
  (не `ru`/`en`) — `IllegalStateException` (без silent fallback).
...
- **`ClaudeImageStager`** — обёртка над `TempFileWriter` (SPI-интерфейс в
...
- **`ClaudeAsyncClientFactory`** — `@Component`, метод `create(workTimeout: Duration)`:
  ...
  val spec = ClaudeClient.async(options)
      .workingDirectory(Paths.get(claudeProperties.workingDirectory))
  if (claudeProperties.cliPath.isNotBlank()) spec.claudePath(claudeProperties.cliPath)
  return spec.build()
  ```
  Ключевое: `workingDirectory(Path)` — **обязательный** параметр SDK 1.0.0
  (`ClaudeClient.AsyncSpec#workingDirectory`, иначе `IllegalArgumentException`).
  `claudePath(String)` — опциональный путь к бинарю. Оба параметра задаются
  на уровне `AsyncSpec` builder-а, **не** `CLIOptions`.
  SDK 1.0.0 **не поддерживает** `startupTimeout` — поле из конфига удалено.
  Клиент short-lived: один вызов — один клиент. `buildEnvMap()` добавляет
  `CLAUDE_CODE_OAUTH_TOKEN`, `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`.
- **`ClaudeInvoker` / `DefaultClaudeInvoker`** — adapter над SDK-вызовом:
  ```kotlin
  suspend fun invoke(prompt: String): String {
      val client = clientFactory.create(workTimeout)
      try {
          client.connect().awaitSingleOrNull()
          return client.query(prompt).text().awaitSingle()
      } finally {
          client.close().awaitSingleOrNull() // Mono<Void>, ClaudeAsyncClient не AutoCloseable
      }
  }
  ```
  `ClaudeAsyncClient` **не** реализует `AutoCloseable` — `.use {}` недопустим.
  `close()` возвращает `Mono<Void>`, подписываемся через `awaitSingleOrNull`.
  `query(prompt).text()` возвращает `Mono<String>` — именно готовый текст,
  не требует manual парсинга `AssistantMessage`/`TextBlock` (устарело через
  `queryAndReceive`).

### Потребление в `core` — ленивый запуск describe-job
...
class RecordingProcessingFacade(
    ...
    suspend fun processAndNotify(...) {
        ...
        // Supplier (а не готовый Deferred) — describe-job стартует только если
        // Telegram-слой реально будет отправлять уведомление (есть подписчики).
        val descriptionSupplier: (() -> Deferred<Result<DescriptionResult>>)? =
            descriptionAgentProvider.getIfAvailable()?.let { agent ->
                val trimmedFrames = request.frames
                    ...
                    .take(descriptionProperties.common.maxFrames)
                if (trimmedFrames.isEmpty()) null
                else ({
                    descriptionScope.async {
                        runCatching { agent.describe(buildDescriptionRequest(trimmedFrames, ...)) }
                    }
                })
            }

        telegramNotificationService.sendRecordingNotification(
            recording, visualizedFrames, descriptionSupplier,
        )
    }
}
```

Telegram-слой вызывает `descriptionSupplier()` только **после** того, как убедится, что уведомление реально уйдёт хотя бы одному получателю (проверка подписчиков/зон). Иначе AI-токены тратятся впустую.

**Ограничение на 10 кадров** реализуется через `take(descriptionProperties.common.maxFrames)` после сортировки по `frameIndex`. Это совпадает с `LOCAL_VIZ_MAX_FRAMES=10` — Telegram всё равно показывает максимум 10 в media group.

**Пустые frames:** если после `take()` получился пустой список — supplier = `null`, describe не запускается.
...
экранируются только `<`, `>`, `&`. Telegram HTML text content **не требует**
`&quot;` для кавычек (только для значений атрибутов тегов, которых у нас нет).

**HTML-escape базового текста:** `DescriptionMessageFormatter` сам вызывает
`htmlEscape()` на `baseText` (task.message, содержащий `camId`/`filePath`)
...
- `bot.sendMediaGroup(...)` возвращает **один** `ContentMessage<MediaGroupContent<MediaGroupPartContent>>`, не `List<...>`. `group.messageId` — id первого сообщения альбома.
- Для редактирования используем suspend-extensions `bot.editMessageText(contentMessage, ...)` / `bot.editMessageCaption(contentMessage, ...)` (как в `ExportExecutor.kt`), а не raw-классы `EditChatMessageText`/`EditChatMessageCaption`.
- `HTMLParseMode` импортируется из `dev.inmo.tgbotapi.types.message.HTMLParseMode` (не `types.ParseMode.HTMLParseMode`).
- `ContentMessage.messageId` имеет тип `MessageId` (value class над Long). `MessageIdentifier` — deprecated typealias на `MessageId`, можно использовать, но лучше `MessageId`. Cast `as MessageIdentifier` — бесполезен.

**Caption и details редактируются независимо** (two separate `try/catch` в `editOne` single-photo ветке) — если caption edit упал, details всё равно обновится.
...
```kotlin
private suspend fun executeWithRetry(prompt: String, request: DescriptionRequest): DescriptionResult {
    val overallStart = timeSource.markNow()
    val totalBudget = commonProperties.timeout
    var jsonRetries = 0
    ...
    while (true) {
        try {
            val rawJson = try {
                ...
                throw exceptionMapper.map(e)   // CancellationException rethrown
            }
            return responseParser.parse(rawJson, request.shortMaxLength, request.detailedMaxLength)
            ...
            val remaining = totalBudget.toKotlinDuration() - elapsed
            if (remaining <= 5.seconds + 2.seconds) {  // 5s delay + minimum attempt duration
        ...
        // RateLimited, Disabled, Timeout — без retry, наружу
    }
}
```

`withTimeout(commonProperties.timeout)` в `describe()` — общий потолок
(60 сек. по умолчанию). `TimeoutCancellationException` явно ловится на
границе `describe()` и оборачивается в `DescriptionException.Timeout`
(см. §5). Retry-loop проверяет остаток бюджета перед `delay(5s)` —
если слишком мало, не уходит в delay, а пробрасывает последнюю ошибку
с корректным типом (Transport вместо ложного Timeout).

### Нормализация SDK-исключений → `DescriptionException`

Выполняется `ClaudeExceptionMapper.map()`.

**Критично:** `CancellationException` (включая `TimeoutCancellationException`) **не** маппится в `Transport` — это бы сломало structured concurrency. Маппер первым делом делает `if (throwable is CancellationException) throw throwable` — отмена корутины остаётся отменой.

`TimeoutCancellationException` от `withTimeout` в `describe()` явно ловится на границе `describe()` (не в mapper'е) и оборачивается в `DescriptionException.Timeout`.

| SDK/рантайм | Тип |
|---|---|
| `CancellationException` / `TimeoutCancellationException` | **rethrow as-is** (не оборачиваем); `TimeoutCancellationException` в `describe()` ловится отдельно и даёт `DescriptionException.Timeout` |
| `TransportException` (SDK) | `Transport` |
| HTTP 429 — по тексту "rate limit" ИЛИ \b429\b + "http"/"status" | `RateLimited` (уточнённый паттерн — избегаем false positive на "code 429") |
| `JsonProcessingException` / пустые поля | `InvalidResponse` |
| Прочие `ClaudeSDKException` | `Transport` (консервативно retryable) |
| Любой `Throwable` (не `CancellationException`) | Оборачиваем в `Transport`, лог WARN с оригинальным cause |
...
что надо обновить `CLAUDE_CODE_OAUTH_TOKEN`. Ротация токена требует рестарта
контейнера (operational concern, не design).

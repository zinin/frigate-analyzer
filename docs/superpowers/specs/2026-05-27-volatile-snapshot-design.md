...
(`ServerState`, 2 поля наружу — `alive`/`lastCheckTimestamp`) overhead заметен, и решение
включить его в scope обсуждалось — оставлено для униформности паттерна, см. §6.

> **Уточнение writer-моделей (per review iter 1, CRITICAL-3).** Изначальная формулировка
> называла ServerState «single-writer» (по scheduler thread). Это неточно: реальная
> картина — `ServerState` имеет до 3 writers (success-callback + error-callback от
> WebClient.subscribe в health-check + `markServerDead` от request-handler thread).
> `Entry` (в ActiveExportRegistry) имеет 2 writers (`attachCancellable` от export
> coroutine + `markCancelling` от cancel coroutine). Соответственно атомарная замена
> через CAS — это **не** «защита от гипотетического будущего», а закрытие реальных
> check-then-act races; см. ServerState `getAndUpdateHealth` API (§4.4) и Entry
> `getAndUpdateState` (§4.3). `TelegramBotSupervisor` и `WatchRecordsTask` ближе к
> single-writer (одна supervisor coroutine + transition-методы из того же контекста),
> но AtomicReference применяется по соображениям unifomity (§1 п.3).

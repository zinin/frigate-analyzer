...
**«Годен» здесь означает «корректно объявлен и имеет статически необходимые входные данные», а не
«запустится».** Отсутствие или неисполняемость CLI даёт WARN на старте и ошибку в рантайме, но
недоступности пресета не создаёт — иначе появился бы новый способ уронить приложение целиком.
Приложение это монитор камер, описания — необязательное обогащение; отключить наблюдение и
уведомления из-за пропавшего бинарника необязательной фичи несоизмеримо дороже пользы.

Асимметрия «нет токена — недоступен, нет CLI — доступен» **историческая, а не принципиальная**: оба
факта статические, и разница лишь в том, что первый роняет старт уже сегодня, а второй нет, и
расширять множество причин отказа не хочется. Практическое следствие: правило «ноль годных валит
старт» ловит в основном claude-пресет без токена, и это узко по построению. Известное ограничение:
при пропавшем CLI строка авторизации на экране `/ai` останется ⚪ или 🟢, потому что отказ придёт не
как `Unauthorized`; диагноз в этом случае даёт только стартовый WARN в логе.
...
64 байта на всё. Он же стоит на кнопке в `/ai`, поэтому ширина кнопки ограничена этой регуляркой, а
длинная `model` попадает только в текстовую строку активного пресета.

Верхнего предела на число пресетов нет, и он не нужен: пресеты не попадают в текст сообщения — они
и есть клавиатура. Длина текста растёт по числу различных **областей авторизации**, а не пресетов,
а Telegram допускает до 100 рядов клавиатуры. Практический предел задаёт читаемость экрана, и
достигается он задолго до технического.
...
data class DescriptionPreset(
    ...
    /** null = пресет годен. Код, а не свободный текст: см. ниже. */
    val unavailableReason: UnavailableReason?,
)

/**
 * Замкнутый набор причин с типизированными аргументами. Свободной строки здесь нет намеренно:
 * значение рисуется в Telegram, и произвольный текст от фабрики — это путь для вывода провайдера
 * (а с ним и для адреса эндпоинта или куска ключа) прямо на экран владельца, вопреки правилу
 * «секреты не попадают в сообщения Telegram». Аргументы поставляет наш собственный код и их форма
 * известна, поэтому конкретика не теряется. Второй довод — локализация: экран локализован весь,
 * и английская строка из фабрики выпадала бы из него.
 *
 * Технические подробности остаются в логе.
 */
sealed interface UnavailableReason {
    object NoToken : UnavailableReason
    data class CliMissing(val path: String?) : UnavailableReason
    data class HomeUnwritable(val path: String) : UnavailableReason
}
...
**Флаг проверяется дважды: в фасаде и внутри supplier-а, непосредственно перед `describe`.**
Гарантия при этом формулируется одной фразой — *выключение отменяет все описания, которые ещё не
начались*. Между сборкой supplier-а и его вызовом лежат очередь уведомлений и rate limiter, так что
одной проверки в фасаде хватило бы лишь на «подействует со следующей записи», — а нажимают эту
кнопку как раз тогда, когда что-то идёт не так прямо сейчас. Вторая проверка стоит поиска в
процессном кэше `AppSettingsService` на пути, который вот-вот потратит секунды и деньги.

Семантика двух рантайм-контролей намеренно разная, и разница объясняется назначением: активный
пресет фиксируется один раз на вызов (повторы не должны разъезжаться по провайдерам, а лог одной
записи обязан называть один пресет), выключатель — аварийный и потому срабатывает как можно позже.
Уже начатое описание не отменяется: отмена edit-job дорога и рискованна.
...
- **Кэш `AppSettingsService` не имеет TTL.** Прямой SQL по `app_settings` мимо сервиса не виден
  ...
  `database.md`.
- **Известная предсуществующая утечка в алерте авторизации.** `DescriptionAuthAlertNotifier.kt:121`
  прикладывает к сообщению владельцу первые 300 символов технической ошибки провайдера. Для BYOK
  эндпоинт может вернуть адрес или фрагмент ключа, то есть правило «секреты не попадают в сообщения
  Telegram» там ничем не обеспечено. Эта работа дефект не вносит и сознательно не расширяет свой
  объём на его исправление: санитизация `detail` — отдельное изменение со своим риском. Записано,
  чтобы не потерялось.

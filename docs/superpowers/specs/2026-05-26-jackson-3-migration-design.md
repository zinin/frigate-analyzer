...
| `modules/core/src/test/.../controller/StatusControllerTest.kt` | **File-comment UPDATE** (line-comments `//`, lines 10-24 в текущем файле — не KDoc `/** */`). Удалить устаревшую ссылку «tools.jackson, NOT our com.fasterxml.jackson-based JacksonConfiguration» — после миграции `JacksonConfiguration` управляет codec'ом через `WebFluxJacksonCodecConfigurer`. Новая формулировка — honest narrative (наш configurer функционально дублирует Boot 4 auto-config; тест — sanity check, не regression guard). Test body **без изменений** (NEW-4 iter-2 consensus). См. § 4.3 ниже и plan Task 7 Step 5(a) для конкретного replacement текста. |
...
**`StatusControllerTest.kt` — только file-comment update (без behavioral расширения):**
- Текущий тест проверяет ISO-8601 формат + структуру JSON-ответа `/status` (включая поля `recordings.success`/`recordings.errors` добавленные master merge PR #31 в 2026-05-26). Это **недостаточно** как regression guard конкретно для нашего `WebFluxJacksonCodecConfigurer`: все наши настройки совпадают с Spring Boot 4 `tools.jackson` defaults, поэтому удаление configurer'а НЕ сломает этот тест.
- **Behavioral regression guard принципиально невозможен** (NEW-4 consensus 5/5 reviewers iter-2): `/status` GET-only без body — `JacksonJsonDecoder` не активируется; даже POST echo-controller через `@TestConfiguration` не дифференцирует наш configurer от Boot defaults, потому что поведение одинаковое.
- **Comment UPDATE:** убрать устаревшее «through Spring Boot 4's WebFlux Jackson codec stack (tools.jackson, NOT our com.fasterxml.jackson-based JacksonConfiguration)». Целевой блок — file-level **line-comments** (`//`) lines 10-24 в текущем файле, не KDoc. Новая формулировка — honest narrative: тест — sanity check, не regression guard; реальный coverage в `WebFluxJacksonCodecConfigurerTest` (identity reflection) и `JacksonConfigurationTest.ApplicationContextRunner` (bean topology). Конкретный replacement текст — в plan Task 7 Step 5(a).
...
- [ ] `StatusControllerTest.kt` file-comment (lines 10-24) обновлён — нет устаревших ссылок на «com.fasterxml-based JacksonConfiguration»; новая формулировка — honest narrative (тест — sanity check, не regression guard на наш configurer). Test body не расширяется (NEW-4 iter-2 consensus: behavioral regression guard принципиально невозможен — поведение совпадает с Boot 4 defaults).

...
| `ActiveExportTracker` (keep) | `telegram/bot/handler/export/` | **Не удаляется.** Продолжает отвечать за dialog-phase chat lock в `/export`: `tryAcquire(chatId)` вызывается до `runDialog()` и гарантирует, что два параллельных `/export`-диалога в одном DM не начнут перехватывать ответы друг у друга. Lock освобождается при cancel/timeout диалога или при передаче в `ExportExecutor.execute()` (где дальнейший lifecycle подхватывает registry). |
...
- `release(exportId)` — удаление из всех индексов (`byExportId`, `byRecordingId`, `byChat`) выполняется внутри `synchronized(entry)` — чтобы между `byExportId.remove` и вторичной очисткой не вклинился конкурирующий `markCancelling`. Безопасно для двойного вызова. Дополнительно: вызывается через `Job.invokeOnCompletion` (как дополнение к `finally` — на случай LAZY-корутины, отменённой до первого suspension point, где `finally` не сработает).
...
| Cancel в Phase 3 (download) | `CancellationException` проходит через `DataBufferUtils.write`; `downloadJobResult` обрабатывает её **явно** отдельным `catch (CancellationException) { cleanup; throw }` до `catch (Exception)` — иначе temp-файл осиротеет (в корутинах `catch (Exception)` не ловит `CancellationException`). |
| Cancel в SENDING (upload в Telegram) | `CancellationException` прерывает multipart upload; `cleanupExportFile` вызывается из `finally` (а не `catch (Exception)`), что гарантирует cleanup. |

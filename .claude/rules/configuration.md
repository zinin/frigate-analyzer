...
`first-scan-period` в `application.yaml` намеренно имеет **пустой** дефолт (`${FIRST_SCAN_PERIOD:}`): пустое значение биндится в `null`, поэтому срабатывает Kotlin-дефолт `firstScanPeriod = watchPeriod`, который видит уже разрезолвленный `watch-period` — из env, profile-yaml, CLI-аргумента или relaxed-имени одинаково. Сама строка нужна, чтобы вообще биндилась короткая переменная `FIRST_SCAN_PERIOD`.

Форма со ссылкой (`${FIRST_SCAN_PERIOD:${application.records-watcher.watch-period}}`) здесь не годится: relaxed-имена гарантированы для биндинга `@ConfigurationProperties`, но не для резолва плейсхолдеров, и `RecordsWatcherPropertiesBindingTest` показал, что такая ссылка отдаёт `P1D` из самого yaml вместо переопределения через `APPLICATION_RECORDSWATCHER_WATCHPERIOD`. `reappear-gap` ниже использует форму со ссылкой безопасно лишь потому, что в имени целевого свойства нет дефиса, который мог бы «съесться»; копировать её в свойство с дефисом в имени нельзя.
